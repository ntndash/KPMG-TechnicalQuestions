...
param(
    ...
    [string]$resourcegroup = "lr-central-app-storage-rg",
    # The name of arm template file
    [Parameter(Mandatory = $false)]
    [string]$lrTemplatefile = "azuredeploy.json",
    # The name of deployment
    [Parameter(Mandatory = $false)]
    [string]$deploymentname = "file_storage_deployment",
...
if ($ValidateOnly) {
    ...
    #Start the 1st deployment to create sql server and database instances
       ...
       <# $x = Get-AzureRmTrafficManagerProfile -ResourceGroupName $resourcegroup
        $x
        $x.Endpoints#>
}
